...
public class FFaplVm implements IVm {
		...
		if(!a.isNull()){
			if(a.typeID() == IJavaType.ARRAY){
				if(b.compareTo(BigInteger.valueOf(Integer.MAX_VALUE)) <= 0){
					this.pushStack(a.getValue(b.intValue()));
				}else{
					Object[] arguments = {b};
	...
	@Override
	public void storeFormalParamValue(int offset) {
		Object val;
		val = this._expressionStack.pop();
	...
	public void isEqual() throws FFaplAlgebraicException {
		...
		if(max[0] instanceof BigInteger){
			...
			if( max[1] instanceof Polynomial){
				this.pushStack(
						new JBoolean(((Polynomial)max[0]).equals(max[1]))
				);
				return;
			}
			...
			if(max[1] instanceof GaloisField){
				this.pushStack(
						new JBoolean(((GaloisField)max[0]).equals(max[1]))
				);
				return;
			}
...
*/
	 ...
	 * @throws FFaplAlgebraicException 
	 */
	private Object add(JString a, JString b) {
 ...
 */
	...
	private Object castTo(int typeID, Object from, Object to) throws FFaplAlgebraicException{
		...
		Object[] arguments = {fromobj.classInfo() , txt};
